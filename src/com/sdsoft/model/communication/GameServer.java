	...
	public void startGame() {
		synchronized (players) {
			...
			for (final Player player : players) {
				...
				// final SGSPacket rulerPacket = new
				// SGSPacket(Action.PickRulerHero);
				// final List<Hero> heros = new
				// ArrayList<Hero>(controller.takeRulerHeros(RulerCardCount));
				// rulerPacket.setHeros(heros);
				// send(rulerPacket, player.getAddress(), player.getPort());
				// }
			}
